...
public class StartJourneyTest {
    ...
    private static final int REPORTING_MANAGER_ID = 7;
   ...
   @Test(priority = 6)
    public void getTodayTourPlanAndWorkType() {
        ...
        String workType = (String) tourPlanDetails.get("workType");
  ...
  //@Test(priority = 7)
    public void getSampleInventoryForMember() {
        int memberId = DataStore.getInt("memberId");
        Response response = PlanUtils.getSampleInventoryByMemberId(memberId, 0, 10, "createdDate", "asc", SALESPERSON_TOKEN);
        String sampleLogId = response.jsonPath().getString("data[0].id");
        DataStore.put("sampleLogId", sampleLogId);
        response.prettyPrint();
    }

  // @Test(priority = 8)
    public void getProductInventoryByClientId() {
        Response response = PlanUtils.getProductInventory(40, 0, 10, "createdDate", "desc", SALESPERSON_TOKEN);
        String productId = response.jsonPath().getString("data[0].productId");
        DataStore.put("productId", productId);
        response.prettyPrint();
    }

//    @Test(priority = 5)
//    public void fetchCombinePlanAndExtractAllLogs() {
//        String visitDate = "2025-06-25";
//        CombinePlanExtractor.extractAllLogsAndPlanIds(SALESPERSON_TOKEN, MEMBER_ID, visitDate);
//    }
